...
- `ROM`(Read Only Memory)
  - 비휘발성
  - **읽기만 가능**한 주기억장치의 일종
    > 💡 ROM이 비휘발성 메모리로 데이터가 영구저장 될 수 있었던 이유는, 데이터를 저장함에 있어서 전류에 의존하지 않고, **바이너리 코드(이진 코드)를 사용**해 개별 셀에 쓰이기 때문이다.<br />
...
> `논리적 주소`: 가상 주소로 각 프로세스마다 독립적으로 할당되는 공간<br/> > `물리적 주소`: 실제 물리 메모리에 적재되는 위치를 식별할 수 있는 주소
...
- CPU가 생성하는 주소를 `논리 주소`
- 메모리가 취급하게 되는 주소(즉, 메모리 주소 레지스터(MAR)에 주어지는 주소)를 `물리 주소`
